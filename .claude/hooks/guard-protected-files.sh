...
if ! command -v jq >/dev/null 2>&1; then
  cat <<'JSON'
{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"Required hook dependency 'jq' is missing; refusing to evaluate protected-files policy. Install jq: apt-get install jq / brew install jq / apk add jq."}}
JSON
  exit 0
fi
