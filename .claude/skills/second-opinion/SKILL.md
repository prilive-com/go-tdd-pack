...
# Minimal redaction (extend by adding patterns to .claude/redact-patterns.txt).
red() {
  awk '
    BEGIN {
...
# Examples: gpt-5.5 (newest frontier; needs ChatGPT auth, not API key)
#           gpt-5.4 (flagship)
#           gpt-5.3-codex (most capable agentic coding model; cheapest reasoning)
...
if [ -z "$response" ]; then
  echo "Codex returned no output (timeout, network, or auth issue). Skipping second opinion."
  exit 0
fi
...
3. Severity-specific rationale rules:
   - **P0**: write at least 3 sentences of rationale (any stance).
   - **P1**: write at least 2 sentences if not plain accept.
...
- Your stance and rationale per finding.
- The proposed plan/code update if any findings led you to change something.

Then either:

- Continue with implementation (if no changes needed or user agrees with your accept/pushback decisions), OR
- Wait for user APPROVED on the updated plan (if you changed Tier 1 spec content — re-enters the TDD ceremony spec gate).
